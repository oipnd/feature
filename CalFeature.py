...
def Cal2Point_vec(Txtdata):
    ...
    if len(TracePos) < 3:
        ...
        vec_mat = [numpy.mean(vec),numpy.std(vec),numpy.var(vec),numpy.median(vec),numpy.min(vec),numpy.max(vec),numpy.max(vec) - numpy.min(vec),numpy.std(vec) / numpy.mean(vec),scit.kurtosis(vec),scit.skew(vec),scit.kurtosis(dis_mat),scit.skew(dis_mat)]
...
def DataFormation(AllData):
    ...
    for i in range(len(AllData)):
        linesAll = AllData[i]
        lines = []
        for lineData in range(len(linesAll)):#去除浏览器信息行
            tmp = linesAll[lineData].split()
            if(len(tmp) != 0 and (tmp[0].startswith("mouse"))):
                lines.append(linesAll[lineData])
        tmp = lines[-1].split()
...
#        print thisSampleData.shape
        ...
        for i in range(len(thisSampleData)-1):
            if ((thisSampleData[i,0]==2 and thisSampleData[i+1,0]==2) or (thisSampleData[i,0]==2 and (thisSampleData[i+1,0]==0 or thisSampleData[i+1,1]==9))):
